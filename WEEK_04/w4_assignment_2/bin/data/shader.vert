...
void main()
{
	...
	float displacementWidth = 200.0;
    float displacementY = sin(sound + (vertex.x / 80.0)) * displacementHeight;
	float displacementX = sin(sound + (vertex.y / 200.0)) * displacementWidth;
	
    vec4 modifiedPosition = position;
    vec3 normal = (gl_NormalMatrix * gl_Normal);
    modifiedPosition.xyz += normal * vec3(displacementX,displacementY,1);
